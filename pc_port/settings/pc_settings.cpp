...
enum Row {
    ...
    ROW_VSYNC,
    ROW_FPS_MODE,
    ROW_CONTROL_MODE,    // NEW: Control mode selection
    ROW_CONTROLS,
    ROW_GAMEPAD,
    ROW_ADVANCED,
    ROW_RESET,
...
unsigned sVideoConfirmTick = 0;
constexpr unsigned sVideoConfirmDurationTicks = 60 * 8; // ~8 s at 60 Hz

// Lazy font state.
Font* sFont = nullptr;
bool sFontTried = false;

int sResolutionIdx = 2; // 1280x720 default
...
int sAdvancedSelection = 0; // 0=sensitivity, 1=stick dead zone, 2=stick invert, 3=c-stick invert
...
void startVideoConfirm() {
    sVideoConfirmTick = sCurrentTick;
...
void saveConfig() {
    ...
    out << "# Pikmin PC port settings (F1 in-game to change)\n";
...
void pollMenuInput() {
    ...
    if (keyWentDown(SDL_SCANCODE_F1)) {
        if (sMenuOpen) {
            ...
            sVideoConfirmActive = false;
            int idx = resolutionIndexFor(pc_window_get_width(), pc_window_get_height());
            sResolutionIdx = idx >= 0 ? idx : 2;
        }
        return;
    }
    ...
    if (sVideoConfirmActive) {
        ...
        unsigned elapsed = sCurrentTick - sVideoConfirmTick;
        if (elapsed >= sVideoConfirmDurationTicks) {
            revertVideoSettings();
            return;
        }
    ...
    if (sInAdvancedSubmenu) {
        ...
        if (up) {
            sAdvancedSelection = (sAdvancedSelection + 4 - 1) % 4;
            return;
        }
        if (down) {
            sAdvancedSelection = (sAdvancedSelection + 1) % 4;
            return;
        }
        if (cancel) {
    ...
    auto cycleResolution = [](int dir) {
        sResolutionIdx = (sResolutionIdx + dir + kResolutionCount) % kResolutionCount;
        sPending.windowWidth = kResolutions[sResolutionIdx].w;
        sPending.windowHeight = kResolutions[sResolutionIdx].h;
    };
    ...
    case ROW_FPS_MODE:
        ...
        break;
    case ROW_CONTROL_MODE:
        if (left) {
            sPending.controlMode = (sPending.controlMode - 1 + 2) % 2;
        } else if (right) {
            sPending.controlMode = (sPending.controlMode + 1) % 2;
        }
        break;
    case ROW_CONTROLS:
        if (ok) {
...
void pc_settings_init(void) {
    loadConfig();
    sPending = sConfig;
    int idx = resolutionIndexFor(sPending.windowWidth, sPending.windowHeight);
    sResolutionIdx = idx >= 0 ? idx : 2;
...
void pc_settings_draw(void) {
    ...
    if (sVideoConfirmActive) {
        unsigned elapsed = sCurrentTick - sVideoConfirmTick;
        int remain = (int)sVideoConfirmDurationTicks - (int)elapsed;
        if (remain < 0) remain = 0;
        int remainS = remain / 60 + (remain % 60 ? 1 : 0);
    ...
    const char* labels[ROW_COUNT] = {
        ...
        "FPS Mode", "Control Mode", "Controls", "Gamepad", "Advanced Settings",
    ...
    const char* controlModeNames[2] = { "Classic (GameCube)", "Mouse Cursor" };
    char controlModeBuf[32];
    snprintf(controlModeBuf, sizeof(controlModeBuf), "%s", controlModeNames[sPending.controlMode >= 0 && sPending.controlMode < 2 ? sPending.controlMode : 0]);

    char valueBuf[8][128];
    snprintf(valueBuf[0], sizeof(valueBuf[0]), "%s",
             modeNames[sPending.displayMode >= 0 && sPending.displayMode < 3 ? sPending.displayMode : 0]);
    snprintf(valueBuf[1], sizeof(valueBuf[1]), "%dx%d", sPending.windowWidth, sPending.windowHeight);
    ...
    snprintf(valueBuf[7], sizeof(valueBuf[7]), "%s", controlModeBuf);

    // Only the first seven rows have computed setting values. Submenu rows
    // previously indexed valueBuf[7..9], displaying arbitrary adjacent memory.
    const char* rowValues[11] = {
        valueBuf[0], valueBuf[1], valueBuf[2], valueBuf[3], valueBuf[4],
        valueBuf[5], valueBuf[6], valueBuf[7], "Open >", "Open >", "Open >",
    };
